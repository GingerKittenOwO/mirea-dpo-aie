...
def head(
    ...
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    n: int = typer.Option(5,help="Количсетво строчек."),
    shrink: bool = typer.Option(False,help="Сжать ли вывод для более удобного вывода в консоль"),
) -> None:
    """
    Вывести первые n значений датасета
    """
    df = _load_csv(Path(path), sep=sep, encoding=encoding)
    
    head_df = df.head(n)
    if shrink:
        head_df = shrink_head(df.head(n))
...
def report(
    ...
    title: str = typer.Option("EDA-отчёт",help="Заголовок отчета (в начале report.md)."),
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть столбцы с большим количеством нулей: **{quality_flags['has_many_zero_values']}**\n\n")
        ...
        if not top_cats:
            ...
            f.write(f"Расчет проводился для топ {top_k_categories} значений.\n\n")

        f.write("## Гистограммы числовых колонок\n\n")
        f.write("См. файлы `hist_*.png`.\n")
        f.write(f"В набор гистограмм включалось {max_hist_columns} колонок.\n")
