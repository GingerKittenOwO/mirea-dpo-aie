...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        if not col.is_numeric:
            if col.unique> summary.n_rows * 0.6:
    ...
    for col in summary.columns:
        if col.is_numeric:
            if col.zeros > summary.n_rows * 0.6:
                high_zero_values+=1
    flags["has_many_zero_values"] = high_zero_values  > 0
    flags["high_zero_count"] = high_zero_values
    ...
    if high_cardinality_count>0:
        score-= 0.02 * high_cardinality_count # вычитаем за каждый стобец 
    if high_zero_values>0:
        score-= 0.05 * high_zero_values
...
def shrink_head(head: pd.DataFrame) -> pd.DataFrame:
    """
    Сжимает head Для более удобного вывода
    """
    head_new = head.copy()
        ...
        if col.count("_")>1:
            ...
            for j in range(len(col)):
                ...
                if flag>0:
                    new_name+=(col[j])
                    flag-=1
            head_new=head_new.rename(columns={col:new_name})
            flag=0
    return head_new
