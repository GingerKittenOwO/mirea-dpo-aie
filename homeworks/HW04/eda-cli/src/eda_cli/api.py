...
# ---------- /quality-flags-from-csv: вывод флагов датасета ----------


@app.post(
    "/quality-flags-from-csv",
    tags=["quality"],
    summary="Вывод флагов для CSV-файла с использованием EDA-ядра",
)
async def quality_from_csv(file: UploadFile = File(...)) -> dict[str,dict[str,bool]]:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает полученные флаги.
    """
...
# ---------- /head: вывод первых n значений ----------


@app.post(
    "/head",
    tags=["visualization"],
    summary="Вывод первых n значений датасета",
)
async def quality_from_csv(file: UploadFile = File(...), n: int = 5) -> str:
    """
    Эндпоинт, который принимает CSV-файл и возвращает head для n строк в формате строки
    """
    ...
    return df_head.to_string(index=True)
